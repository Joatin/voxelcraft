...
impl CompressedChunk {
    ...
    pub async fn save(&self, storage: Arc<dyn Storage>) -> Result<(), Box<dyn Error>> {
        storage.save_chunk_blocks(&self.compressed_block_data, self.chunk_position).await?;
        Ok(())
    }

    pub async fn load(storage: Arc<dyn Storage>, chunk_position: ChunkPosition) -> Result<Self, Box<dyn Error>> {
    ...
    pub fn try_into_chunk(self) -> Result<Chunk, Box<dyn Error>> {
        let (chunk, _) = bincode::decode_from_slice(&self.compressed_block_data, config::standard())?;
        Ok(chunk)
    }
}