...
use wgpu::{CommandBuffer, Device, CommandEncoderDescriptor, RenderPassDescriptor, RenderPipeline, Buffer};
use std::error::Error;
use voxelcraft_server::client::Client;
use crate::interface::{Message, IN_GAME_HUD_PAGE_ROUTE};
use uuid::Uuid;
use futures::{stream, StreamExt, TryStreamExt};
use crate::chunk::{MeshableChunk, ChunkMesh};
...
impl LocalGame {
    ...
    pub fn new(device: Arc<Device>, pipelines: &Arc<HashMap<String, RenderPipeline>>, block_texture_map: &Arc<BlockTextureMap>, camera_utils: Arc<CameraPipelineUtils>) -> Self {
        ...
        tokio::spawn(Self::start_connection_process(Arc::clone(&client), Arc::clone(&device), Arc::clone(&chunk_meshes), Arc::clone(&messages), Arc::clone(&is_loading)));
    ...
    async fn start_connection_process(client: Arc<LocalClient>, device: Arc<Device>, chunk_meshes: Arc<Mutex<Vec<ChunkMesh>>>, messages: Arc<std::sync::Mutex<Vec<Message>>>, is_loading: Arc<AtomicBool>) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        let mut meshes = stream::iter(chunks_to_mesh).map(|position| {
            ...
            async move {
                ...
                let handle = tokio::spawn(async move {
                    chunk.create_mesh(&device).await
                });
                ...
                Self::send_loading_message(&messages, "Building chunks in the player vicinity", Some(progress));
                Result::<_, Box<dyn Error + Send + Sync>>::Ok(mesh)
            }
        }).buffer_unordered(num_cpus::get() * 2).try_collect::<Vec<_>>().await?;

        log::info!("Successfully meshed {} chunks in {:?}", chunks_to_mesh_count, Instant::now().duration_since(instant_start));
    ...
    fn send_loading_message(messages: &Arc<std::sync::Mutex<Vec<Message>>>, message: &str, progress: Option<f32>) {
        log::info!("Sending loading message: '{}'", message);
        {
            let mut lock = messages.lock().unwrap();
            lock.push(
                Message::GameLoadingMessage(message.to_string(), progress)
            )
        }
    }
    ...
    fn finnish_loading(messages: &Arc<std::sync::Mutex<Vec<Message>>>) {
        {
            let mut lock = messages.lock().unwrap();
            lock.push(
                Message::GameLoadingMessage("".to_string(), None)
            );
            lock.push(
                Message::Navigate {
    ...
    pub async fn process_events(client: Arc<LocalClient>) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    async fn render_chunk_meshes(chunk_meshes: Arc<Mutex<Vec<ChunkMesh>>>, pipelines: Arc<HashMap<String, RenderPipeline>>, block_texture_map: Arc<BlockTextureMap>, camera_utils: Arc<CameraPipelineUtils>, render_context: RenderContext) -> Vec<CommandBuffer> {
...
impl Game for LocalGame {
        ...
        if !self.is_loading.load(Ordering::Relaxed) {
            ...
            let handle = tokio::spawn(async move {
                let mut command_buffers = {
                    ...
                    camera_utils.update(&render_context.device, &mut encoder, &camera, &projection).await;

                    vec![encoder.finish()]
                };

                command_buffers.append(&mut Self::render_chunk_meshes(chunk_meshes, pipelines, block_texture_map, camera_utils, render_context).await);
    ...
    fn resize(&mut self, size: Size) {
        self.projection.resize(size.width as u32, size.height as u32)
    }
