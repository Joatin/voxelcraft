use crate::{Chunk, ChunkStorage, ChunkFactory};
use std::mem;
...
#[derive(Debug)]
pub struct ChunkCache<P: 'static + Hash + Eq + Send + Sync, S: ChunkStorage<P>, F: ChunkFactory<P, Chunk=Chunk<T, SIZE>>, T: 'static, const SIZE: usize> {
...
impl<P: 'static + Hash + Eq + Clone + Send + Sync, S: ChunkStorage<P>, F: ChunkFactory<P, Chunk=Chunk<T, SIZE>>, T: 'static + Encode + Decode, const SIZE: usize> ChunkCache<P, S, F, T, SIZE> {
    pub fn new(max_in_mem_chunk_byte_size: usize, max_compressed_byte_size: usize, storage: Arc<S>, factory: F) -> Self {
    ...
    pub async fn borrow_chunk<C: FnOnce(&Chunk<T, SIZE>) -> R, R>(&self, position: &P, callback: C) -> Result<R, Box<dyn Error>> {
    ...
    async fn load_from_compressed_cache(&self, position: &P) -> Result<Option<Arc<RwLock<Chunk<T, SIZE>>>>, Box<dyn Error>> {
    ...
    async fn load_from_storage(&self, position: &P) -> Result<Option<Arc<RwLock<Chunk<T, SIZE>>>>, Box<dyn Error>> {
    ...
    async fn insert_chunk(&self, position: &P, chunk: Chunk<T, SIZE>) -> Arc<RwLock<Chunk<T, SIZE>>> {
        let mut lock = self.chunks.write().await;
        let locked_chunk = Arc::new(RwLock::new(chunk));

        // TODO: Pop if cache is full

        if lock.insert(position.clone(), Arc::clone(&locked_chunk)).is_some() {
