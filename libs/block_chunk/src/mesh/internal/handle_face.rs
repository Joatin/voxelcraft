...
pub fn handle_face<T, FC: FnOnce(&BlockOffset<SIZE>) -> Face<SIZE>, C: Send + Sync + Fn(&T) -> Option<BlockDescriptor>, const SIZE: usize>(chunk: &Chunk<T, SIZE>, describe_callback: &C, mesh: &mut Vec<Face<SIZE>>, transparent_mesh: &mut Vec<Face<SIZE>>, descriptor: &BlockDescriptor, position: &BlockOffset<SIZE>, neighbour_position: Option<BlockOffset<SIZE>>, face_callback: FC) {
