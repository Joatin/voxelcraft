use crate::mesh::{BlockDescriptor, MeshResult, Face, FaceDirection, Corner};
use crate::{Chunk, BlockOffset};
use crate::mesh::internal::handle_block::handle_block;

pub async fn fast_mesh<T, C: Send + Sync + Fn(&T) -> Option<BlockDescriptor>, const SIZE: usize>(chunk: &Chunk<T, SIZE>, describe_callback: C) -> MeshResult<SIZE> {
    let mut mesh = vec![];
    ...
    for x in 0..SIZE {
        for y in 0..SIZE {
            for z in 0..SIZE {
                handle_block(chunk, &describe_callback, &mut mesh, &mut transparent_mesh, &mut unhandled, x, y, z);
            }
        }
    }
...
mod tests {
    ...
    async fn it_should_give_correct_amount_of_faces() {
        ...
        let result = chunk.fast_mesh(|_| Some(BlockDescriptor {
